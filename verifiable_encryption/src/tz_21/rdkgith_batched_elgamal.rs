...
//! The only difference with implementation in the module `rdkgith` is that a more efficient variant of Elgamal called batched
...
// TODO: This contains a lot of duplicate code from module `rdkgith`. Abstract the encryption scheme to reduce duplication
...
use dock_crypto_utils::{
    aliases::FullDigest,
    elgamal::BatchedHashedElgamalCiphertext,
...
> {
    ...
    pub ciphertexts: [(u16, BatchedHashedElgamalCiphertext<G>); NUM_PARTIES_MINUS_THRESHOLD],
    ...
    > RdkgithProof<NUM_PARTIES, THRESHOLD, NUM_PARTIES_MINUS_THRESHOLD, G>
{
    ...
    pub fn new<R: RngCore, D: Digest + FullDigest>(
        ...
        let enc_rands = [G::ScalarField::rand(rng); NUM_PARTIES];
        let mut cts: [BatchedHashedElgamalCiphertext<G>; NUM_PARTIES] =
            [(); NUM_PARTIES].map(|_| BatchedHashedElgamalCiphertext::<G>::default());
        ...
        cfg_iter_mut!(cts).enumerate().for_each(|(i, ct)| {
            let shares_i = cfg_iter!(shares).map(|s| s.0[i].share).collect::<Vec<_>>();
            *ct = BatchedHashedElgamalCiphertext::new_given_randomness_and_window_tables::<D>(
                &shares_i,
                &enc_rands[i],
                &enc_key_table,
                &enc_gen_table,
        ...
        let mut ciphertexts: [(u16, BatchedHashedElgamalCiphertext<G>);
            NUM_PARTIES_MINUS_THRESHOLD] = [(); NUM_PARTIES_MINUS_THRESHOLD]
            .map(|_| (0, BatchedHashedElgamalCiphertext::<G>::default()));
        let mut shares_and_enc_rands: [(u16, Vec<G::ScalarField>, G::ScalarField); THRESHOLD] =
            [(); THRESHOLD].map(|_| (0, Vec::with_capacity(witness_count), G::ScalarField::zero()));
        ...
        for i in 0..NUM_PARTIES {
            if indices_to_hide.contains(&(i as u16)) {
                ciphertexts[ctx_idx] = (i as u16, cts[i].clone());
                ctx_idx += 1;
            } else {
                shares_and_enc_rands[s_idx].0 = i as u16;
                shares_and_enc_rands[s_idx].2 = enc_rands[i];
                for j in 0..witness_count {
                    shares_and_enc_rands[s_idx].1.push(shares[j].0[i].share);
                }
                s_idx += 1;
            }
        }
        ...
        cfg_iter_mut!(cts).enumerate().for_each(|(i, ct)| {
            if hidden_indices.contains(&(i as u16)) {
                ...
                for (k, s, r) in &self.shares_and_enc_rands {
                    ...
                    if i as u16 == *k {
                        *ct = BatchedHashedElgamalCiphertext::new_given_randomness_and_window_tables::<D>(s, r, &enc_key_table, &enc_gen_table);
                        break
                    }
                }
            }
        ...
        cfg_iter_mut!(compressed_cts)
            ...
            .for_each(|(i, ct)| {
                ...
                let deltas = cfg_iter!(opened_indices)
                    ...
                    .collect::<Vec<_>>();
                ct.eph_pk = self.ciphertexts[cphtx_idx].1.eph_pk;
                ct.encrypted = cfg_into_iter!(0..witness_count)
                    .map(|j| {
                        let mut e = self.ciphertexts[cphtx_idx].1.encrypted[j]
...
impl<const SUBSET_SIZE: usize, G: AffineRepr> CompressedCiphertext<SUBSET_SIZE, G> {
    pub fn decrypt<D: FullDigest>(
        ...
        for i in 0..SUBSET_SIZE {
            ...
            let shared_secret = (self.0[i].eph_pk * dec_key).into_affine();

            let witnesses = cfg_into_iter!(0..witness_count)
                .map(|j| {
                    let otp = self.1[i]
                        * BatchedHashedElgamalCiphertext::otp::<D>(&shared_secret, j as u32);
                    self.0[i].encrypted[j] - otp
                })
                .collect::<Vec<_>>();

...
macro_rules! impl_common_funcs {
    ( $key_type:ident, $setup_param_variant:ident) => {
        ...
        pub fn get_proving_key<'a, G: AffineRepr>(
            ...
            setup_params: &'a [SetupParams<E, G>],
            st_idx: usize,
        ) -> Result<&'a $key_type<E::G1Affine>, ProofSystemError> {
            extract_param!(
                setup_params,
                &self.proving_key,
...
impl<E: Pairing> AccumulatorMembership<E> {
    ...
    impl_common_funcs!(MembershipProvingKey, VbAccumulatorMemProvingKey);
}
...
impl<E: Pairing> AccumulatorNonMembership<E> {
    ...
    impl_common_funcs!(NonMembershipProvingKey, VbAccumulatorNonMemProvingKey);
}

use crate::{
    bagheri_pvss::{same_base::EncryptedShare, validate_threshold},
    common::ShareId,
    error::SSError,
    shamir_ss,
};
...
/// in the increasing order of their ids in the context of secret sharing
...
> {
    ...
    let r = n_rand(rng, total).collect::<Vec<_>>();
    ...
    let z = (0..total as usize)
        .map(|i| r[i].clone() + (shares.0[i].share * d))
        .collect::<Vec<_>>();
    Ok((
        enc_shares,
        Proof {
...
impl<F: PrimeField> Proof<F> {
    ...
    pub fn verify<G: AffineRepr<ScalarField = F>, D: Digest>(
        ...
        expect_equality!(
            self.resp.len(),
            total as usize,
            SSError::UnexpectedNumberOfResponses
        );
        let mut chal_bytes = vec![];
        for (i, pk) in public_keys.into_iter().enumerate() {
            ...
            // pk * r_i - y_i * d
            let t = (pk * self.resp[i]) - (enc_share_i.share * self.challenge);
            pk.serialize_compressed(&mut chal_bytes)?;
            t.serialize_compressed(&mut chal_bytes)?;
...
pub mod tests {
    use super::*;
    use crate::{bagheri_pvss::Share, common};
